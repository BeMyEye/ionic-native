...
import { CordovaCheck, CordovaInstance, Plugin, InstanceProperty, InstanceCheck, IonicNativePlugin, checkAvailability } from '@ionic-native/core';
    ...
    return new Observable((observer) => {
      this._objectInstance.on(eventName, (...args: any[]) => {
        if (args[args.length - 1] instanceof GoogleMaps.getPlugin().BaseClass) {
          args[args.length - 1] = args[args.length - 1].id;
        }
        observer.next(args);
      });
    });
  ...
  constructor(element: string | HTMLElement, options?: GoogleMapOptions) {
    ...
    if (checkAvailability(GoogleMaps.getPluginRef(), null, GoogleMaps.getPluginName()) === true) {
      if (element instanceof HTMLElement) {
        ...
        const dummyObj: any = new (GoogleMaps.getPlugin().BaseClass)();
        this._objectInstance = dummyObj;
        const onListeners: any[] = [];
        const oneListeners: any[] = [];
        const _origAddEventListener: any = this._objectInstance.addEventListener;
        const _origAddEventListenerOnce: any = this._objectInstance.addEventListenerOnce;
  ...
  addKmlOverlay(options: KmlOverlayOptions): Promise<KmlOverlay | any> {
    return new Promise<KmlOverlay>((resolve, reject) => {
      this._objectInstance.addKmlOverlay(options, (kmlOverlay: any) => {
        if (kmlOverlay) {
          resolve(new KmlOverlay(this, kmlOverlay));
        } else {
          reject();
        }
      });
